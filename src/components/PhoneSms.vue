		...
		<el-form-item label="验证码" prop="sms">
			<div class="flex">
				...
				<el-button
					...
					:disabled="btnDis"
					@click="sendSmsCode('phoneSms', formData.phone)"
				>
					{{ btnMes }}
				</el-button>
			</div>
...
export default {
	...
	methods: {
		async getSmsCode(phone) {
			...
			// 				typ: 'post'
			// 			};
			// 			const getResult = await this.$store.dispatch('apiRequest', requestConfi
      ...
      // }
		...
		time() {
			...
			this.btnSetInterval = setInterval(() => {
				...
				if (this.btnTime > 0) {
					if(this.btnTime<10){
						this.btnTime = '0'+this.btnTime
					}
					this.btnMes = `重新发送(${this.btnTime}s)`;
				} else {
					this.btnMes = '获取短信验证码';
					this.btnTime = 60;
					this.btnDis = false;
